...
bool llava_image_embed_make_with_clip_img(clip_ctx * ctx_clip, int n_threads, const clip_image_u8 * img, float ** image_embd_out, int * n_img_pos_out) {
    ...
    // Add validation for MINICPMV models
    if (clip_is_minicpmv(ctx_clip)) {
        LOG_INF("MINICPMV model: n_img_pos = %d, max_patches = %d\n", n_img_pos, num_max_patches);
        if (n_img_pos > num_max_patches * clip_n_patches(ctx_clip)) {
            LOG_ERR("%s: number of image positions (%d) exceeds maximum allowed (%d)\n", 
                    __func__, n_img_pos, num_max_patches * clip_n_patches(ctx_clip));
            free(image_embd);
            return false;
        }
    }
...
struct llava_embd_batch {
    ...
    llava_embd_batch(float * embd, int32_t n_tokens, llama_pos pos_0, llama_seq_id seq_id) {
        ...
        for (int i = 0; i < n_tokens; i++) {
            ...
            batch.logits  [i] = true;  // Set all logits flags to true for MINICPMV
        }
    }
};

bool llava_eval_image_embed(llama_context * ctx_llama, const struct llava_image_embed * image_embed, clip_ctx * ctx_clip, int n_batch, int * n_past) {
    int n_embd  = llama_n_embd(llama_get_model(ctx_llama));
    
    // Process all tokens in a single batch for MINICPMV models
    if (clip_is_minicpmv(ctx_clip)) {
        float * embd = image_embed->embed;
        llava_embd_batch llava_batch = llava_embd_batch(embd, image_embed->n_image_pos, *n_past, 0);
